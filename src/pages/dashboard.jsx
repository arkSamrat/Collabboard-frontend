...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(`${BASE_URL}/task-api?email=${user.email}`, {
  ...
  const handleDragEnd = async ({ source, destination, draggableId }) => {
    ...
    try {
      const res = await axios.put(`${BASE_URL}/tasks/${updatedTask._id}`, updatedTask, {
        withCredentials: true,
      });

      if (res.data.success) {
        setTasks(prev => prev.map(t => (t._id === updatedTask._id ? res.data.updated : t)));
      }
    } catch (err) {
  ...
  const handle_logout = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/logout-api`, {
        withCredentials: true,
      });
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <input type="text" placeholder="Title" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} style={{ marginRight: '10px' }} />
        <input type="text" placeholder="Description" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} style={{ marginRight: '10px' }} />
      ...
      {user.role === 'admin' && (
        <div style={{ marginTop: '40px' }}>
          ...
          <ul style={{ fontFamily: 'monospace' }}>
            {logs.map((log) => (
              <li key={log._id}>
                <strong>{log.email}</strong> {log.action} task <em>"{log.taskTitle}"</em> at {new Date(log.timestamp).toLocaleTimeString()}
              </li>
            ))}
          </ul>
        </div>
...
const styles = {
  page: { minHeight: '100vh', backgroundColor: '#f4f6f9', padding: '20px' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' },
  title: { fontSize: '2rem', color: '#333' },
  logout: { padding: '10px 20px', backgroundColor: '#dc3545', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' },
  board: { display: 'flex', gap: '20px', flexWrap: 'wrap' },
  column: { flex: '1', minWidth: '280px', maxWidth: '32%', backgroundColor: '#fff', borderRadius: '8px', padding: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', boxSizing: 'border-box' },
  columnTitle: { fontSize: '1.2rem', marginBottom: '15px', fontWeight: 'bold' },
};
